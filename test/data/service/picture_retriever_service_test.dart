...
class MockPictureService extends Mock implements PictureRetrieverUseCase {}
...
''',
  ...
  group('Picture Retriever Service', () {
    test('check if the fetchImage function is returning an image', () async {
      when(mockPictureService.fetchImage()).thenAnswer((_) async => pictures);

      final result = await mockPictureService.fetchImage();

      expect(result, pictures);
    });
