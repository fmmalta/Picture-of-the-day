...
void main() {
  group('Picture of the day local response', () {
    test('check if the picture is been stored', () async {
      ...
      when(localRepository.storeLastestImage(_picture))
          .thenAnswer((_) async => true);

      final response = await localRepository.storeLastestImage(_picture);
    ...
    test('check if the picture is been retrieved', () async {
      final localRepository = MockMockLocalRepository();
      when(localRepository.retrieveLastestImage())
          .thenAnswer((_) async => _picture);

      final response = await localRepository.retrieveLastestImage();

      expect(response, _picture);
    });

    test('check if the value is null if there is not picture stored', () async {
      final localRepository = MockMockLocalRepository();
      when(localRepository.retrieveLastestImage())
